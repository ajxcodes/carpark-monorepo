...
public class ParkingServiceTests
{
    ...
    [Fact]
    public async Task ParkAsync_GivenLessThanFiveAvailableSpaces_ShouldReturnParkingResponseWithSurgePricing()
    {
        // Arrange
        var request = new ParkingRequest { VehicleReg = "TEST-123", VehicleType = VehicleType.SmallCar };
        var availableSpace = new ParkingSpace { Id = Guid.NewGuid(), Number = 1 };
        _spaceRepository.GetFirstAvailable().Returns(availableSpace);
        _spaceRepository.GetAll().Returns([availableSpace]);
        var session = new ParkingSession
        {
            Vehicle = new Vehicle { Registration = request.VehicleReg, Type = VehicleType.SmallCar },
        ...
        session.AddSurgePricing();
        _sessionRepository.Get(Arg.Any<string>()).Returns(session);
        var startParkResponse = await _sut.ParkAsync(request);
        startParkResponse.ShouldNotBeNull();
        startParkResponse.VehicleReg.ShouldBe(request.VehicleReg);
        startParkResponse.SpaceNumber.ShouldBe(availableSpace.Number);
        //startParkResponse.TimeIn.ShouldBeInRange(DateTime.UtcNow.AddSeconds(-5), DateTime.UtcNow.AddSeconds(5));
        var exitRequest = new ParkingExitRequest { VehicleReg = "TEST-123" };

        // Act
        var response = await _sut.ExitAsync(exitRequest);
