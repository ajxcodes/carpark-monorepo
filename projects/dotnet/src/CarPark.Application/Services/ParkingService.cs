    ...
    IChargeCalculatorService chargeService) : IParkingService
{
    ...
    {
        var availableSpaces = await GetSpacesAsync();
        var addSurgePricingRequired = availableSpaces.AvailableSpaces < 5;
        ...
        if(addSurgePricingRequired) parkingSession.AddSurgePricing();
